...
from factors.hotelling import hotelling
...
# data = np.matrix('0.670 0.581 0.154; 0.581 0.630 0.439; 0.154 0.439 0.420')
# input_data = np.matrix('1 0.10 0.75 0.20; 0.10 1 0.10 0.85; 0.75 0.10 1 0.05; 0.20 0.85 0.05 1')
...
# ======================================================================================================================
# main factors - hotelling

alpha = np.empty(shape=(data.shape[0], factors_num))
lambdas = np.empty(shape=factors_num)
F = np.empty(shape=(data.shape[0], factors_num))
output = np.empty(shape=(data.shape[0], factors_num + 2))

RH = data.__copy__()
for i in range(0, factors_num):
    alpha[:, i][:, None], lambdas[i] = hotelling(RH)
    F[:, i] = alpha[:, i] * sqrt(lambdas[i]) / norm(alpha[:, i])
    # print("============================")
    # print("=================== data for f" + str(i+1))
    # print("alpha" + str(i + 1) + " = " + str(alpha[:, i]))
    # print("lambda" + str(i + 1) + " = " + str(lambdas[i]))
...
# print("data to plot - after rotation: " + str(F1))
